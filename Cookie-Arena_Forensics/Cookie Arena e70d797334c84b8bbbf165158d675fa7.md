...
- **File: girl-xinh.jpg**
    ...
    *Describe: Is real photo?*
    

Mình mở HxD xem thử file thì 2 byte đầu tiên là `4D 5A` (`MZ`). Đây là 2 byte đầu tiên của header file thực thi 16-bit DOS MZ executable, với extension là .exe. Nên mình đổi extension và chạy file trong cmd.

![Screenshot 2023-06-18 194520.png](img/Screenshot_2023-06-18_194520.png)
...
- **File: Travis.pcapng**
    ...
    *Describe:  PCAP stands for "Packet Capture." It refers to a file format and a common method for capturing and storing network traffic data. PCAP files are widely used in network analysis and troubleshooting to capture packets flowing through a network interface.*
...
![Screenshot 2023-06-18 195611.png](img/Screenshot_2023-06-18_195611.png)
...
- **File: thumb.db**
    ...
    *Describe: In Windows, `thumbs.db` files are database files containing the small images displayed when you view a folder in Thumbnail view (as opposed to Tile, Icon, List, or Detail view). These files are automatically generated by Windows, and there is no harm in deleting them or excluding them from system backups.*
...
![Screenshot 2023-06-18 213011.png](img/Screenshot_2023-06-18_213011.png)
...
- **File: WebCam.png**
    ...
    *Describe:  You get a photo of a place taken from a online camera Use `https://holidaylivecam.com/camera/search?tag%5B%5D=schools_universities` to find the place*
...
![Screenshot 2023-06-19 125738.png](img/Screenshot_2023-06-19_125738.png)
...
Theo mô tả của đề bài thì mình mở file bằng Audacity và chỉnh là xem ở chế độ Spectrogram.
...
![Screenshot 2023-06-19 140419.png](img/Screenshot_2023-06-19_140419.png)
...
- ***Folder: Repository***
    ...
    *Describe: Đội CSIRT của chúng tôi được yêu cầu xử lý sự cố trên một máy chủ mà chúng tôi đã xử lý xong cách đây vài ngày. Vì vậy chúng tôi tin rằng vẫn còn một cơ chế persistence nào đó của kẻ tấn công mà chúng tôi đã bỏ sót*
    

Đây là lần đầu tiên mình thấy các file có extension như vậy, nên khá là lạ lẫm.

![Screenshot 2023-06-20 185338.png](img/Screenshot_2023-06-20_185338.png)
...
![Screenshot 2023-06-20 210613.png](img/Screenshot_2023-06-20_210613.png)
...
![Screenshot 2023-06-27 004241.png](img/Screenshot_2023-06-27_004241.png)
...
- **File: dismantling.pcap**
    ...
    *Describe: Be careful with Alternative Protocol*
...
- ********************File: s3cr3t.png********************
    ...
    *Describe:  Doing with image files should be the first step for anyone who learning cybersecurity.*
...
- ********************File: legit.png********************
    ...
    *Describe: Gần đây trong một sự cố, chúng tôi phát hiện kẻ tấn công đã đánh cắp dữ liệu của chúng tôi bằng cách nhúng nó vào trong một tệp hình ảnh và sử dụng các chức năng upload để đưa dữ liệu ra khỏi hệ thống của chúng tôi một cách an toàn. Bạn có thể tìm xem dữ liệu được ẩn giấu là gì không?*
...
# SS2-Tin hoc văn phòng

- ********************File: Challenge.doc********************
    
    *Describe: Sau khi tham gia một khóa Tin học văn phòng cơ bản, Hòa đã có thể tự tạo một tệp tài liệu độc hại và anh ta có ý định sẽ dùng nó để hack cả thế giới*
...
- ********************File: NTUSER.DAT********************
    ...
    Describe: *Hòa thấy hiện tượng lạ mỗi khi anh ta khởi động máy tính. Anh ta nghĩ rằng việc tải các video không lành mạnh gần đây đã khiến máy tính của anh ta bị hack.*
...
- ********************File: trivialFTP.pcapng********************
    ...
    *Describe: Việc những nhân viên của một công ty X sử dụng các giao thức không an toàn để kết nối và truyền tải tập tin từ xa đã tạo cơ hội cho những kẻ tấn công Man in the Middle và đánh cắp dữ liệu quan trọng của công ty*
...
![packet cuối cùng có độ dài data nhỏ hơn 512](img/Untitled%2019.png)
...
Mình up code đã được deobf ở đây: [link](https://github.com/ik-nown/Write_up_CTF/blob/main/Cookie-Arena_Forensics/UnderControl/powershell.ps1)
